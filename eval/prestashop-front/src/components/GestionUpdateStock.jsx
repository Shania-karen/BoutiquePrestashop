 ...
 */
export default function GestionUpdateStock({ onClose }) {
  ...
  const [report, setReport] = useState([]); // Stock adjustment report
  ...
  const handleApplyDelta = async (categoryId) => {
    ...
    if (isNaN(delta) || delta === 0) {
      showNotification("tsy mandeha ", "error");
      return;
    }
    ...
    try {
      ...
      for (const pid of Object.keys(productStocks)) {
        ...
        if (delta > 0) {
        // PLUS DE DIVISION : on applique exactement le delta tapé à CHAQUE déclinaison
        for (const item of items) {
            // 1. On prévoit d'ajouter exactement ce que vous avez tapé dans la case
            let plannedAdd = delta; 
            
            let actualAdd = plannedAdd;
            let nonRealised = 0;
            ...
            if (hasLimit) {
              // Si le stock actuel + l'ajout prévu dépasse notre plafond
              if (item.currentQty + plannedAdd > limitNum) {
                  // On ajoute seulement ce qu'il manque pour atteindre la limite (ou 0 si déjà dépassé)
                  actualAdd = Math.max(0, limitNum - item.currentQty);
                  
                  // Le reste va dans "Non-Réalisé"
                  nonRealised = plannedAdd - actualAdd;
              }
            }
            // -----------------------------------

            // 3. On applique les vrais chiffres calculés
            item.newQty = item.currentQty + actualAdd;
            item.realised = actualAdd;
            item.nonRealised = nonRealised;
            item.deltaApplied = actualAdd;
        }
      } else if (delta < 0) {
  ...
  if (loading && categories.length === 0) {
    const catMatch = categoryToAdd ? extractValue(product.id_category_default) === categoryToAdd : true;
    const catDelete = categoryToDelete ? extractValue(product.id_category_default) === categoryToDelete : true;
    return catMatch && catDelete;
  }
  ...
  return (
    ...
    <form onSubmit={async (e) => {
      ...
      setReport([]);
      if (categoryToAdd) {
        await handleApplyDelta(categoryToAdd);
      }
      if (categoryToDelete) {
        await handleApplyDelta(categoryToDelete);
      }
      if (!categoryToAdd && !categoryToDelete) {
      ...
      {(() => {
        const selectedCatInfo = displayedCategories.find(c => extractValue(c.id) === categoryToAdd);
        const isCategoryEmpty = selectedCatInfo ? selectedCatInfo.prodCount === 0 : false;

        return (
          <input
            type="number"
            placeholder="Ex: 5"
            value={categoryToAdd ? (deltaValues[categoryToAdd] ?? '') : ''}
            onChange={e => setDeltaValues(prev => ({ ...prev, [categoryToAdd]: e.target.value }))}
            style={inputQtyStyle}
            disabled={!categoryToAdd || isCategoryEmpty}
          />
        );
      })()}
      <p>Supprimer stock </p>
      ...
      {(() => {
        const selectedCatDelete = displayedCategories.find(c => extractValue(c.id) === categoryToDelete);
        const isCategoryEmpty = selectedCatDelete ? selectedCatDelete.prodCount === 0 : false;

        return (
          <input
            type="number"
            placeholder="Ex: 5"
            // Pour l'affichage, on montre toujours un chiffre positif (Math.abs enlève le signe -)
            value={categoryToDelete && deltaValues[categoryToDelete] ? Math.abs(deltaValues[categoryToDelete]) : ''}
            onChange={e => {
              // Au moment de la frappe, on force le chiffre en NÉGATIF !
              const typedValue = e.target.value;
              const negativeValue = typedValue ? -Math.abs(typedValue) : '';
              
              setDeltaValues(prev => ({ ...prev, [categoryToDelete]: negativeValue }));
            }}
            style={inputQtyStyle}
            disabled={!categoryToDelete || isCategoryEmpty}
          />
        );
      })()}
      </div>          
          ...
          <button type="submit" disabled={loading} className="btn-login">
            {loading ? 'En cours de chargement...' : 'Valider'}
          </button>
      ...
      {report.length > 0 && (
        <div style={{ marginTop: '30px' }}>
          <h2 style={{ fontSize: '20px', marginBottom: '15px' }}>Rapports d'exécution</h2>
          
          {report.map((r, index) => (
            <div key={index} style={{ marginBottom: '20px', padding: '20px', background: '#f8f9fa', border: '1px solid #e0e0e0', borderRadius: '4px' }}>
              <h3 style={{ marginTop: 0, marginBottom: '10px', fontSize: '18px', color: '#333' }}>
                Action : <span style={{ color: r.deltaRequested > 0 ? '#2e7d32' : '#c62828' }}>
                  {r.deltaRequested > 0 ? 'Ajout' : 'Suppression'}
                </span> - Catégorie : <span style={{ color: '#000' }}>{r.categoryName}</span>
              </h3>
              <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#555' }}>
                Vous avez demandé un ajustement de <strong>{r.deltaRequested > 0 ? `+${r.deltaRequested}` : r.deltaRequested}</strong> sur chacun des <strong>{r.numProducts}</strong> produits de cette catégorie.<br/>
                Soit un ajustement <strong>GLOBAL</strong> attendu de <strong>{r.deltaRequested > 0 ? '+' : '-'}{r.globalDeltaRequested}</strong> unités au total.
              </p>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', background: '#fff' }}>
                <thead>
                  <tr style={{ background: '#eee', borderBottom: '2px solid #ddd' }}>
                    ...
                    <th style={{ padding: '10px', textAlign: 'center', fontWeight: '600', color: '#555' }}>Ajustement</th>
                ...
                <tfoot>
                  <tr style={{ background: '#f5f5f5', borderTop: '2px solid #ccc' }}>
                    <td colSpan="4" style={{ padding: '12px 10px', textAlign: 'right', fontWeight: 'bold', fontSize: '14px' }}>
                      TOTAL GÉNÉRAL ({r.realisedTotal + r.nonRealisedTotal}) :
                    </td>
